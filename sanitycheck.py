...
def run_sanity_check(test_dir):
    ...
    print('''Please enter the path to the file
          that contains your test cases for
    ...
    def f2(x):
        return inspect.isfunction(f1(x))

    def f3(x):
        return inspect.getsource(f1(x))

    test_function_names = list(filter(
                               lambda x: f2(x) and not x.startswith('__'),
                               dir(module)))

    test_functions_for_get = list(filter(lambda x: f3(x).find('.get(') != -1,
                                  test_function_names))

    test_functions_for_post = list(filter(lambda x: f3(x).find('.post(') != -1,
                                   test_function_names))
    ...
    if not test_functions_for_get:
        ...
        for func in test_functions_for_get:
            ...
            cond_1 = (source.find('.json') != -1)
            cond_2 = (source.find('json.loads') != -1)
            if cond_1 or cond_2:
    ...
    if not test_functions_for_post:
        ...
        for func in test_functions_for_post:
            ...
            cond_1 = (source.find('json.loads') != -1)
            cond_2 = (source.find('.json') != -1)
            if cond_2 or cond_1:
        ...
        cond_1 = len(test_functions_for_post) >= 2
        cond_2 = COUNT_POST_METHOD_TEST_FOR_INFERENCE_RESULT < 2
